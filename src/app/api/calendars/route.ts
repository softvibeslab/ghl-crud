...
// GET /api/calendars - List calendars with pagination and filters (RBAC protected)
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const result = await services.calendars.findAll(pagination, filtersWithTenant)
...
// POST /api/calendars - Create a new calendar (RBAC protected)
export async function POST(request: NextRequest) {
  // Check permission to create calendars
  const authResult = await requirePermission(request, 'calendars', 'create')
  if (authResult instanceof NextResponse) {
    return authResult
  }
